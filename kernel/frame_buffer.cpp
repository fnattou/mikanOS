...
Error FrameBuffer::Copy(Vector2D<int> pos, const FrameBuffer& src) {
    ...
    const uint32_t dst_width = config_.horizontal_resolution;
    const uint32_t dst_height = config_.vertical_resolution;
    const uint32_t src_width = config_.horizontal_resolution;
    const uint32_t src_height = config_.vertical_resolution;
