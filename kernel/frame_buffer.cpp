...
Error FrameBuffer::Initialize(const FrameBufferConfig& config) {
    ...
    if (config_.frame_buffer) {
        ...
        const size_t bufferBytes = bytes_per_pixel * 
...
Error FrameBuffer::Copy(Vector2D<int> dst_pos, const FrameBuffer& src,
    ...
    const Rectangle<int> copy_area = dst_outline & src_outline & src_area_shifted;
    const Vector2D<int> src_start_pos = copy_area.pos - (dst_pos - src_area.pos);
...
void FrameBuffer::Move(Vector2D<int> dst_pos, const Rectangle<int>& src) {
    const int bytes_per_pixel = BytesPerPixel(config_.pixel_format);
    const int bytes_per_scan_line = BytesPerScanLine(config_);
