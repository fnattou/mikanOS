...
Rectangle<T> operator&(const Rectangle<T>& lhs, const Rectangle<U>& rhs) {
  const Vector2D<T> lhs_end = lhs.pos + lhs.size;
  const Vector2D<T> rhs_end = rhs.pos + rhs.size;
  if (lhs_end.x < rhs.pos.x || lhs_end.y < rhs.pos.y ||
      rhs_end.x < lhs.pos.x || rhs_end.y < lhs.pos.y) {
    return {{0, 0}, {0, 0}};
  }

  Vector2D<T> new_pos = ElementMax(lhs.pos, rhs.pos);
  Vector2D<T> new_size = ElementMin(lhs_end, rhs_end) - new_pos;
